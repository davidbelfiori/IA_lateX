...
La bilanciatura di un albero binario di ricerca (BST) consiste nel mantenere la struttura dell'albero in modo tale che la profondità di ogni nodo sia il più vicina possibile alla profondità media dell'albero, il \textbf{fattore di bilanciamento} si indica con \(\beta(v)\). Ciò migliora l'efficienza delle operazioni di ricerca e inserimento, che sono operazioni \( O(log\: n) \) in un BST bilanciato.
\\
\\
Esistono diversi tipi di alberi bilanciati: 
\begin{itemize}
\item\textbf{ Alberi AVL} : Gli alberi AVL sono BST bilanciati in cui la differenza tra la profondità dei sottoalberi sinistro e destro di ogni nodo non supera mai 1 (\(\beta(v) \leq 1\)). Il bilanciamento è ottenuto tramite \textbf{rotazioni}.
...
I B-alberi sono generalmente "piatti" ovvero la loro altezza generalmente è tra 1 e 2
...
\begin{itemize}
	...
	\item Le \textbf{chiavi} vengono mantenute solo nei nodi interni dell'albero
	\item Le foglie sono costituite da nodi speciali \verb*|Nil|
...
\end{figure}
\textbf{Memorizzazione alberi Red-Black}: in aggiunta ai campi classici di un albero binario ovvero:
\begin{itemize}
	\item Tree \verb*|parent|
	\item Tree \verb*|left|
	\item Tree \verb*|right|
	\item \textbf{int} \verb*|color|
	\item Item \verb*|key|
	\item Item \verb*|value|
\end{itemize}
\textbf{Altezza nera di un nodo \(v\))}: l'altezza nera \(bh(v)\) di un nodo \(v\) è il numero di nodi neri lungo ogni cammino da \(v\)(escluso) ad ogni foglia (inclusa) del suo sottoalbero \\
\textbf{Altezza nera di un albero Red-Black}: L'altezza nera di un albero Red-Black è pari all'altezza nera della sua radice 
\newpage
\textbf{Inserimento}: E' possibile che durante la modifica di un albero Red-Black le condizioni di bilanciamento risultino violate.
\\
...
\begin{itemize}
	...
	\item Operando dei ribilanciamenti dell'albero tramite \textbf{rotazioni} rispettivamente \textbf{rotazione destra} e \textbf{rotazione sinistra}

\end{itemize}
\begin{figure}[h]
	\centering
	\includegraphics[width=0.5\linewidth]{photo/rotazionedx.png}
\end{figure}

\begin{algorithm}
	\caption{Rotazione a sinistra }
	\begin{algorithmic}[1]
		\Function{Tree RotateLeft}{$Tree \: \: x$}
		\State $Tree y \gets x.right;$
		\State $Tree p \gets x.parent;$
		...
		\State $y.parent \gets p;$
		\If{$p \not= \text{nil}$}
		\If{$p.left = x $}
		\State $p.left \gets y ;$
		\Else
		\State $p.right \gets y;$
		\EndIf
		\EndIf
		\EndFunction
	\end{algorithmic}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.9\linewidth]{photo/InserimentoNodoAlbero}
	\label{fig:inserimentonodoalbero}
	\newline
	\\
	\includegraphics[width=0.9\linewidth]{photo/balanceinsert}
	\label{fig:balanceInsert}
	\newline
	\\
	\includegraphics[width=0.9\linewidth]{photo/removeNode}
	\label{fig:removeNode}
	\newline
	\\
	\includegraphics[width=0.9\linewidth]{photo/balanceDelete}
	\label{fig:balanceDelete}
\end{figure}

\newpage
\section{Ordinamento}
\subsection{Problema dell'ordinamento}
\textbf{Problema dell'ordinamento:}Abbiamo una collezione di oggetti $R_i$ ciascuno con una chiave $K_i$.
\\
Le chiavi ammettono una relazione di ordinamento: \textbf{Tricotomia} e la \textbf{transitività}
\\
\\
\textbf{Perché abbiamo bisogno di un inseme ordinato ?} Perché la ricerca in un inseme ordinato ha un costo di ordine $0(log\: n)$, quindi avere una sequenza in ordine può far risparmiare tempo. 
\subsubsection{Permutazioni}
Una \textbf{Permutazione} è una sequenza di \textit{n} di ogetti distinguibili tra loro in uno specifico ordine.
\\
\\
Se l'insieme degli oggetti è l'inseme {0,1,2,3,...} possiamo usarli come \textit{indici} per accedere un altro insieme di oggetti 
\subsection{Bubble sort}
Il \textbf{bubble sort} è un semplice algoritmo di ordinamento che ripetutamente scorre la lista, o vettore, confronta i due elementi vicini e li scambia se si trovano nell'ordine sbagliato. Lo scorrimento attraverso gli elementi prosegue fino a quando la lista non è completamente ordinata.
\begin{figure}[h]
	\centering
	\includegraphics[width=1\linewidth]{photo/bubblesort}
	\label{fig:bubblesort}
\end{figure}
Analizzando la performance dell’algoritmo di ordinamento possiamo osservare che:
\begin{itemize}
\item nel caso peggiore, cioè quando tutti gli elementi contenuti sono da ordinare, presenta una complessità computazionale $O(n^2)$ dove n corrisponde al numero di elementi da ordinare nella lista.
\item nel caso migliore, ovvero quando la lista è completamente ordinata e non si deve svolgere alcuno scambio, possiede una complessità computazionale $O(n)$. In questo caso il bubble sort risulta più efficiente di altri algoritmi di ordinamento, in quanto l’abilità di verificare che la lista sia ordinata è implementata direttamente nell’algoritmo.
\end{itemize}
\newpage
\subsection{Insertion Sorting}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=1\linewidth]{photo/insertion}
	\label{fig:insertion}
\end{figure}
\\
\textbf{Quale è il costo?}
\begin{itemize}
	\item ad ogni iterazione del ciclo \verb*|For i = 0... n-1|, abbiamo un ciclo intero,ed un paio di assegnazioni
	\item Il ciclo interno viene eseguito per valori \verb*|j| che possono andare (nel caso peggiore) \textit{i} a 0, mentre nel caso migliore non verrà eseguita nessuna operazione.

\end{itemize}
Ogni iterazione del ciclo esterno ha un costo potenzialmente diverso. Quindi nel caso peggiore

\[ 
	opcnt = \bigg(\sum_{i=0}^{n-1}1 + 1 \bigg)+ \bigg(\sum_{i=0}^{n-1} \sum_{j=0}^{i-1} 1 \bigg)
	\le 2n + \sum_{i=0}^{n-1} (i + 1) 
	= 3n + \frac{(n-1)n}{2} 
	= O(n^2)
 \]
...
\begin{itemize}
	...
	Il caso peggiore si verifica quando il pivot viene scelto in modo tale da dividere l'array in due parti sbilanciate ogni volta. In questo caso, quicksort impiega $O(n^2)$ operazioni per ordinare l'array, che è la stessa complessità di bubble sort e insertion sort.
\end{itemize}
\newpage
...
\begin{figure}[h]
	\centering
	\includegraphics[width=1\linewidth]{photo/mergeSort}
	\label{fig:mergesort}
	\includegraphics[width=1\linewidth]{photo/merge}
	\label{fig:merge}
\end{figure}
\subsubsection{Complessità Computazionale del Mergesort}
La complessità computazionale del mergesort è analoga a quella del Quicksort, dato che ad ogni passo ricorsivo sto spezzando la sequenza iniziale in due sottosequenze eguali, per ciò $T(n)=O(nlog(n)))$ 
\\
\\
\textbf{Osservazione:}
	Il \textbf{Mergesort} è un algroritmo migliore del quicksort in termini di complessità nel caso peggiore   
	$O(nlog(n))$ per il mergesort e   $O(n^2)$ per il Quicksort.
	Tuttavia, nel caso medio il Quicksort è più veloce
\end{document}
